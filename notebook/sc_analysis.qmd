...
The stability summaries favor the PFlog, no-cell-cycle-HVG branch at 30 PCs
and resolution 0.3. Across the comparison grid, this choice has local mean and
minimum ARI of 0.833 and 0.603, mean NMI of 0.877, and mean best Jaccard of
0.882. These summaries describe agreement across parameter choices; they do
not by themselves assign biological identities to clusters.

**Choice 2**: I use PFlog normalization, no-cell-cycle-HVG selection, 30 PCs,
and resolution 0.3 for the source clustering. This source clustering has nine
...
![Chosen full-dataset UMAP for PFlog normalization, cell-cycle HVGs retained, 30 PCs, resolution 0.3.](figures/umap_pflog_no_filter_cc_dims30_by_cluster_pflog_no_filter_cc_dims30_res0_3.png){#fig-umap-resolution-sweep}
...
![Per-cluster cell-type module scores and descriptive p27 annotation for the full source PFlog clustering (no cell-cycle filtering, 30 PCs, resolution 0.3). The body shows `cell_type_marker_genes` module scores from the RNA `data` layer, z-scored across clusters and clipped to ±2; the top strip shows the cluster-level p27 enrichment z-score from PFlog expression after within-Mouse × Condition cluster-label permutations.](figures/cell_type_module_p27_heatmap_pflog_pflog_no_filter_cc_dims30_res0.3.png){#fig-mg-full-module-p27-heatmap}

Cluster 2 (573 cells; microglia+p27) meets both the microglia-marker and high-p27
exclusion criteria. Clusters 7 (95 cells; p27) and 8 (22 cells; p27) meet the
high-p27 exclusion criterion. No cluster met the photoreceptor exclusion
criteria. I filtered out clusters 2, 7, and 8, retaining 3,456 cells, then
...
PFlog, no-cell-cycle-HVG, 20-PC, resolution-0.5 clustering has eight clusters,
with local mean and minimum ARI of 0.704 and 0.658. All eight clusters span
both conditions, and seven span all mice. I use this clustering for downstream
figures and differential analysis. The cell-cycle-filtered branch at the same
20-PC, resolution-0.5 setting is a sensitivity comparison.

![MG-selected UMAP labeled by Leiden cluster ID for PFlog normalization, cell-cycle HVGs retained, 20 PCs, resolution 0.5.](figures/mg_selected_cluster_umap_pflog_mg_selected_no_filter_cc_dims20_res0.5.png){#fig-mg-selected-cluster-umap}

![MG-selected UMAP colored by condition for PFlog normalization, cell-cycle HVGs retained, 20 PCs, resolution 0.5.](figures/mg_selected_condition_umap_pflog_mg_selected_no_filter_cc_dims20_res0.5.png){#fig-mg-selected-condition-umap}
...
* Cluster 4 is enriched for neuronal-associated genes (`Chrna4`, `Ascl1`, `Hes6`).
* Cluster 5 is enriched for neuronal-associated genes (`Cabp5`, `Stmn3`, `St8sia3`).
* Cluster 7 is enriched for cell-cycle-associated genes (`Esco2`, `Tk1`, `Neil3`).
* Cluster 8 has highly specific markers including `Cnmd`, `Scrg1`, and `Gja1`.

![Top cluster-enriched FindAllMarkers genes for the MG-selected chosen clustering. Dot size shows the fraction of cells detecting each gene; color uses six diverging row-z-score bins for average scaled expression from the Seurat data layer, with endpoint bins labeled $\leq -2$ and $\geq 2$.](figures/mg_selected_cluster_marker_dotplot_data_pflog_mg_selected_no_filter_cc_dims20_res0.5_top5.png){#fig-mg-selected-find-all-markers-dotplot}
...
![Per-cell marker-gene heatmap for the MG-selected PFlog clustering. Cluster numbers are shown on the top annotation bar; cell-type labels are shown on the left; marker genes are listed beside the heatmap.](figures/cell_type_marker_heatmap_pflog_pflog_mg_selected_no_filter_cc_cells_dims20_res0.5.png){#fig-mg-selected-cell-type-marker-heatmap}
...
![MG-selected 3 × 3 PFlog feature UMAP grid for the no-cell-cycle-filtered branch.](figures/mg_selected_feature_umap_pflog_pflog_mg_selected_no_filter_cc_dims20_res0.5.png){#fig-mg-selected-feature-umap-grid}

![Per-cell Ascl1 / Hes6 coexpression for the MG-selected no-cell-cycle-filtered PFlog branch, colored by Leiden cluster. Axes show PFlog-normalized expression.](figures/mg_selected_ascl1_hes6_coexpression_pflog_mg_selected_no_filter_cc_dims20_res0.5.png){#fig-mg-selected-ascl1-hes6-coexpression}

As a complementary check, I also reviewed the matching PFlog,
cell-cycle-filtered 20-PC, resolution-0.5 branch. The UMAPs show the Leiden
cluster IDs and condition overlay for that branch.

![MG-selected UMAP labeled by Leiden cluster ID for PFlog normalization with cell-cycle HVGs filtered before PCA, 20 PCs, resolution 0.5.](figures/mg_selected_cluster_umap_pflog_mg_selected_filter_cc_dims20_res0.5.png){#fig-mg-selected-filter-cc-cluster-umap}

![MG-selected UMAP colored by condition for PFlog normalization with cell-cycle HVGs filtered before PCA, 20 PCs, resolution 0.5.](figures/mg_selected_condition_umap_pflog_mg_selected_filter_cc_dims20_res0.5.png){#fig-mg-selected-filter-cc-condition-umap}
...
Primary DESeq2 tested 24,601 genes and found 442 FDR-significant genes for
E-Stim versus `p27CKO`: 131 increased and 311 decreased with E-Stim. Nine
curated marker-list genes were significant. `Glul` (Müller glia), `Ccn1`
(activated Müller glia), `Hes6` (neurogenic progenitor), `Grm6` (rod bipolar),
and `Scgn` (cone bipolar) increased with E-Stim. `Mcm2`, `Mcm6`, and `Pcna`
(proliferative) and `Rcvrn` (photoreceptor) decreased. The paired sensitivity
DE analysis found 141 FDR-significant genes. The strongest design-robust
curated marker signals are concordant in the primary and paired-sensitivity
analyses: increased `Glul` and `Ccn1` and decreased `Mcm2` and `Mcm6`. `Hes6`,
`Pcna`, `Grm6`, `Scgn`, and `Rcvrn` are primary-model hits but do not remain
FDR-significant in the paired sensitivity. `Serpina3n` is significant only in
the paired sensitivity, so I treat it as paired-subset support for activated
Müller glia rather than a primary all-sample marker hit.
