...
`Seurat::AddModuleScore()` using `cell_type_marker_genes`, then handled the
p27 (CDKN1B) expression/detection signal separately. The marker rule removes
microglia or photoreceptor calls only when the top module score is at least 0.5
and exceeds the runner-up score by at least 0.25. The p27 rule removes clusters
with expression and detection BH-adjusted Wilcoxon $q < 0.05$ and at least 20%
detected cells. In this run, cluster 4 met both microglia and p27 rules,
cluster 7 met the microglia rule, and clusters 9 and 10 met the p27 rule,
leaving 4,713 cells for the `mg-selected` dataset. Because p27 filtering
conditions the branch on p27 biology, interpret the downstream results as the
p27-low/negative MG-enriched branch requested here.
...
This heatmap is descriptive cluster annotation, not condition-level evidence. Module scores use RNA `data` by default to mirror `Seurat::AddModuleScore()` and the selection scoring above, while the p27 strip uses the PFlog layer to match the p27 expression signal used for cluster review. The p27 z-score describes whether each cluster is high or low relative to labels permuted within the same Mouse × Condition samples, so the null preserves per-sample composition.
...
![Per-cluster cell-type module scores and descriptive p27 annotation for the MG-selected PFlog clustering (cell-cycle HVGs retained, 30 PCs, resolution 0.3). Body scores use the RNA `data` layer; the top strip uses PFlog p27 values and the same within-Mouse × Condition permutation null as the full-source heatmap.](figures/cell_type_module_p27_heatmap_pflog_pflog_mg_selected_no_filter_cc_dims30_res0.3.png){#fig-mg-selected-module-p27-heatmap}

This per-cluster heatmap makes the `mg-selected` cluster-level identity signals directly comparable without adding a condition-level test. It shows `cell_type_marker_genes` module scores from RNA `data`, z-scored across retained clusters, with the descriptive PFlog p27 enrichment z-score on top. It complements the UMAP and the per-cell marker heatmap and does not replace the clustering, abundance, or differential analyses.
...
cluster abundance in p27CKO + E-Stim versus p27CKO, ordered left-to-right from
the highest E-Stim enrichment to the lowest; colors distinguish enriched or
depleted clusters with significant pooled cell-level Fisher tests after Holm
correction.
...
only, DD only, or both), and gene-name labels are shown only for genes that are
FDR-significant in one or both DE/DD tests using repelled text labels
(`ggrepel::geom_text_repel()`).

![DE and DD effect-size comparison for genes tested in both analyses. Points are genes; x shows the DE effect size, y shows the DD effect size, color shows the FDR significance category (neither, DE only, DD only, or both), and gene-name labels are shown only for genes that are FDR-significant in one or both DE/DD tests using repelled text labels (`ggrepel::geom_text_repel()`).](figures/mg_selected_de_dd_effect_scatter.png){#fig-mg-selected-de-dd-effect-scatter}
