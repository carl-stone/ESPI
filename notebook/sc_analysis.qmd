...
The marker heatmap below shows the selected PFlog, non-filtered clustering
(`cluster_pflog_no_filter_cc_dims50_res0.3`). Columns are cells split by Leiden
cluster, with cluster blocks hierarchically ordered by their marker-expression
profiles. Rows are marker genes grouped by their expected cell-type labels.
Colors show each gene's row z-score across cells, clipped to $\pm 2$.

![Per-cell marker-gene heatmap for the selected PFlog, non-filtered clustering. Cluster numbers are shown on the top annotation bar; cell-type labels are shown on the left; marker genes are listed beside the heatmap.](figures/cell_type_marker_heatmap_pflog_pflog_no_filter_cc_cells_dims50_res0.3.png){#fig-cell-type-marker-heatmap}
