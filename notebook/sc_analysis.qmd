...
![Top cluster-enriched FindAllMarkers genes for the MG-selected chosen clustering. Dot size shows the fraction of cells detecting each gene; color shows average scaled expression from the Seurat data layer. Marker rankings are descriptive because cells, not mice, are the units in FindAllMarkers.](figures/mg_selected_cluster_marker_dotplot_data_pflog_mg_selected_no_filter_cc_dims30_res0.3_top5.png){#fig-mg-selected-find-all-markers-dotplot}

### Differential expression

I ran pseudobulk differential expression on the `mg-selected` object using
Mouse × Condition samples as the statistical unit. The primary DESeq2 design
`~ condition` is the main condition-level analysis and keeps all six Mouse ×
Condition pseudobulk samples. The paired `~ mouse + condition` analysis on mice
10 and 3 is a sensitivity check only, because mouse 30 is E-Stim only and mouse
33 is control only.
...
The differential-detection (DD) analysis uses the pseudobulk `edgeR_NB_optim`
workflow (Gilis et al., BMC Genomics 26:886, 2025) via `muscat::pbDS(method =
"DD")`. Per gene and Mouse × Condition sample we count cells with non-zero
expression, apply the workflow's internal filter that drops genes detected in
>= 90% of cells across samples, add a cellular-detection-rate normalization
...
paired sensitivity uses `~ mouse + condition` on mice 10 and 3.

Primary muscat DD tested 36,468 genes and found no FDR-significant genes or
marker-list hits, so the primary evidence does not support condition-level
changes in the fraction of cells detecting curated marker genes. The paired
sensitivity DD tested 34,880 genes and found 40 FDR-significant genes, with no
marker-list hits. Because this result uses only mice 10 and 3 and is not
supported by primary DD, I treat it as a within-paired-mice sensitivity signal,
not a general all-sample DD conclusion.
