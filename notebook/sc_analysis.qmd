...
minimum ARI of 0.801 and 0.647, mean NMI of 0.851, and mean best Jaccard of
0.859. These summaries describe agreement across parameter choices; they do
...
![Chosen full-dataset UMAP for PFlog normalization, cell-cycle HVGs retained, 30 PCs, resolution 0.3.](figures/umap_pflog_no_filter_cc_dims30_by_cluster_pflog_no_filter_cc_dims30_res0.3.png){#fig-umap-resolution-sweep}
...
exclusion criteria. Clusters 7 (84 cells; p27) and 8 (22 cells; p27) meet the
high-p27 exclusion criterion. No cluster met the photoreceptor exclusion
criteria. I filtered out clusters 2, 7, and 8, retaining 3,248 cells, then
...
All eight clusters span both conditions, and seven span all mice. I use this
...
* Cluster 4 is enriched for marker genes including `Ascl1`, `Chrna4`, and `Hes6`.
* Cluster 5 is enriched for marker genes including `St8sia3`, `Stmn3`, and `Ina`.
* Cluster 7 is enriched for cell-cycle-associated genes (`Esco2`, `Neil3`, `Tk1`).
* Cluster 8 has highly specific markers including `Gja1`, `Scrg1`, and `Cnmd`.
...
Primary DESeq2 tested 24,231 genes and found 427 FDR-significant genes for
E-Stim versus `p27CKO`: 124 increased and 303 decreased with E-Stim. Nine
curated marker-list genes were significant. `Glul` (Müller glia), `Ccn1` and
`Il6` (activated Müller glia), `Hes6` (neurogenic progenitor), and `Grm6` (rod
bipolar) increased with E-Stim. `Mcm2`, `Mcm6`, and `Pcna` (proliferative) and
`Rcvrn` (photoreceptor) decreased. The paired sensitivity DE analysis found 129
...
`Ccn1` and decreased `Mcm2` and `Mcm6`. `Il6`, `Hes6`, `Pcna`, `Grm6`, and
`Rcvrn` are primary-model hits but do not remain FDR-significant in the paired
sensitivity. `Serpina3n` is significant only in the paired sensitivity, so I
treat it as paired-subset support for activated Müller glia rather than a
