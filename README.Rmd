...
branch at 20 PCs and Leiden resolution 0.3. The downstream commands below
reproduce the source and MG-selected figures, marker ranking, and Mouse ×
Condition pseudobulk analysis. MG preprocessing computes 50 PCs so the 30- and
50-PC sensitivity candidates remain available.
...
The shorter MG recipes still default to historical 30- or 50-PC candidate
columns. Pass the explicit 20-PC identifiers above for the current chosen
analysis. Use `just --list` to inspect every parameterized figure, marker, and
MG-selected recipe.
