...
- `module_scores.tsv` from `exploratory/milo/module-score-milo-da.R` with primary settings
  `k = 60` and `prop = 0.04`.
...
```bash
Rscript exploratory/neurogenic-proportion/analysis.R
```
...
Saved results moved here without recomputation. `bootstrap_settings.tsv` retains
the original run's output-directory string as historical provenance; it is not
