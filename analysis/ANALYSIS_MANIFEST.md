...
| MG-selected analysis branch | `scripts/filter-mg-subset.R`, `scripts/summarize-mg-selected-grid.R`, `scripts/plot-mg-selected-figures.R`, `scripts/run-mg-selected-de.R`, `notebook/sc_analysis.qmd` | R pipeline + notebook | active | Scores cluster marker modules with `Seurat::AddModuleScore()`, removes configured confident microglia/photoreceptor clusters and Cdkn1b-high clusters, regenerates PFlog MG-selected clustering/figures, and runs Mouse × Condition pseudobulk DESeq2 plus differential-detection and GO follow-ups. Current run removed microglia-score clusters 4/7 and Cdkn1b-high clusters 4/9/10. |
