#!/usr/bin/env bash
# Project-owned wrapper for the synced Mycelium Stop hook.
# It snapshots semantic provenance files, runs the core hook unchanged, repairs
# placeholder clobbering, then returns the core hook's stdout and status.
set -uo pipefail

REPO_ROOT=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
CORE_HOOK="$REPO_ROOT/skills/core/hooks/mycelium-stop-check.sh"
GUARD="$REPO_ROOT/tools/mycelium-provenance-guard.py"

TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/mycelium-stop-wrapper.XXXXXX") || exit 1
cleanup() {
  rm -rf "$TMP_DIR"
}
