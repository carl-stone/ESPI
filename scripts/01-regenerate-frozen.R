...
# Regenerate frozen source and MG-selection preprocessing artifacts.
# The default run covers counts, QC, four source preprocessing branches, the
# source clustering grid, MG selection, and both MG preprocessing branches.
# Set ESPI_REGENERATION_START to mg-selection to load the configured source RDS
# and skip construction, QC, source preprocessing, and source clustering.
# Run 01b-cluster-mg-sensitivity.R afterward to rebuild MG clustering outputs.

suppressPackageStartupMessages({
  here::i_am("scripts/01-regenerate-frozen.R")
...
if (!regeneration_start %in% c("all", "mg-selection")) {
  stop(
    "ESPI_REGENERATION_START must be 'all' or 'mg-selection'.",
    call. = FALSE
  )
}
seed <- config$seed
...
if (
  any(!dir.exists(required_writable_dirs)) ||
    any(file.access(required_writable_dirs, 2L) != 0L)
) {
  stop(
    "Refusing frozen regeneration; required object directories are not writable: ",
    paste(required_writable_dirs, collapse = ", "),
    call. = FALSE
  )
}
...
if (regeneration_start == "all") {
  ...
  representative_plots <- lapply(resolutions, function(resolution) {
    column <- paste0("cluster_pflog_filter_cc_dims50_res", resolution)
    Seurat::DimPlot(
      representative,
...
cdkn1b_stats <- lapply(cluster_levels, function(cluster) {
  ...
  data.frame(
    ...
    cdkn1b_expression_p = if (dplyr::n_distinct(cdkn1b_expression) == 1L) {
      1
    } else {
      stats::wilcox.test(
    ...
    cdkn1b_detection_p = if (dplyr::n_distinct(cdkn1b_detected) == 1L) {
      1
    } else {
      stats::wilcox.test(
...
for (filter_cc in filter_states) {
  ...
  branch_sobj@misc$preprocessing$source_input <- config$selected$source$path
...
message(
  "Frozen MG preprocessing complete from ",
  regeneration_start,
  ": source cells=",
  ncol(mg_source),
  "; MG cells=",
